...
import com.justpass.app.data.webview.InvalidCredentialsException
import com.justpass.app.data.webview.WebViewAuthenticator
...
class AttendanceRepository(private val context: Context) {
    ...
    suspend fun fetchStudentBiodata(): com.justpass.app.data.model.StudentBiodata? {
        val rollNumber = securePrefs.rollNumber ?: return null
        val password = securePrefs.password
    ...
    suspend fun prefetchForAI() = coroutineScope {
        ...
        try {
            ...
            if (resultData is Result.Success) {
                val entries = gson.fromJson(resultData.data, Array<com.justpass.app.data.model.GradeEntry>::class.java)
