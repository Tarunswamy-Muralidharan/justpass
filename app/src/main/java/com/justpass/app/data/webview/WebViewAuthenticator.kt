package com.justpass.app.data.webview
...
import com.justpass.app.data.model.CircularListResponse
import com.justpass.app.data.model.SignedUrlResponse
import com.justpass.app.data.model.TimetableResponse
import com.justpass.app.data.model.TokenResponse
...
class WebViewAuthenticator(private val context: Context) {
    ...
    var cachedAuthToken: String?
        get() = _cachedAuthToken ?: com.justpass.app.data.local.SecurePreferences.getInstance(context).accessToken.also { _cachedAuthToken = it }
        set(value) {
            _cachedAuthToken = value
            com.justpass.app.data.local.SecurePreferences.getInstance(context).accessToken = value
        }
    private var _cachedAuthToken: String? = null

    var cachedMeetingsToken: String?
        get() = _cachedMeetingsToken ?: com.justpass.app.data.local.SecurePreferences.getInstance(context).meetingsAccessToken.also { _cachedMeetingsToken = it }
        set(value) {
            _cachedMeetingsToken = value
            com.justpass.app.data.local.SecurePreferences.getInstance(context).meetingsAccessToken = value
        }
    ...
    suspend fun loginViaAuthCodeFlow(username: String, password: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            val prefs = com.justpass.app.data.local.SecurePreferences.getInstance(context)
     ...
     * API: GET /sis/students/{rollNumber}
     */
    suspend fun fetchStudentProfile(rollNumber: String): com.justpass.app.data.model.StudentBiodata? {
        val token = cachedAuthToken ?: return null
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (connection.responseCode == 200) {
                    ...
                    com.justpass.app.data.model.StudentBiodata.fromJson(org.json.JSONObject(json))
                } else {
                ...
                webView.webViewClient = object : WebViewClient() {
                    override fun onPageFinished(view: WebView?, url: String?) {
                        ...
                        if (url?.contains(LOGIN_URL_PATTERN) == true) {
                            ...
                            val escapedPass = (com.justpass.app.data.local.SecurePreferences.getInstance(context).password ?: "").replace("\\", "\\\\").replace("'", "\\'")
                            val jsLogin = """
                                (function() {
     ...
     */
    suspend fun fetchCircularPdfUrl(attachment: com.justpass.app.data.model.CircularAttachment): kotlin.Result<String>? {
        val token = cachedMeetingsToken ?: return null
